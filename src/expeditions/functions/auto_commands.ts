...
export const remindTime:Command = { 
  run:(ctx) => {
    if(ctx.expedition.secondsLeft && ctx.expedition.secondsLeft < 1800 && !ctx.expedition.variables.has('_30minleft_'+ctx.player.name)) {
      ctx.expedition.variables.set('_30minleft_'+ctx.player.name, true)
